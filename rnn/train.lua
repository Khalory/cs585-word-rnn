...
require 'util.OneHot'
require 'util.misc'
--local WordSplitMinibatchLoader = require 'util.WordSplitMinibatchLoader'
local WordSplitMinibatchLoader = require 'util.WordSplitMinibatchLoader'
...
local loader = WordSplitMinibatchLoader.create(opt.data_dir, opt.batch_size, opt.seq_length, split_sizes)
...
os.execute(string.format('mkdir %s/%s/', opt.data_dir, opt.checkpoint_dir))
...
for i = 1, iterations do
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local savefile = string.format('%s/%s/lm_%s_epoch%.2f_%.4f.t7', opt.data_dir, opt.checkpoint_dir, opt.savefile, epoch, val_loss)
